...
\begin{itemize}
    ...
    \item \texttt{keycloack} — configuração do servidor Keycloak.
\end{itemize}
...
\begin{tikzpicture}[
    ...
    pstep/.style={rectangle, draw, rounded corners=3pt, fill=blue!10,
                 minimum width=2.5cm, minimum height=0.75cm, font=\small, align=center},
    arr/.style={-{Stealth[length=5pt]}, thick},
    lbl/.style={font=\scriptsize}
]
\node[pstep] (push)   {Push para\\main};
\node[pstep, right=1.2cm of push]  (build)  {Build \&\\Testes};
\node[pstep, right=1.2cm of build] (sonar)  {Análise\\SonarCloud};
\node[pstep, right=1.2cm of sonar] (docker) {Build\\Docker};
\node[pstep, right=1.2cm of docker] (reg)   {Publish\\GHCR};
\node[pstep, right=1.2cm of reg]   (deploy) {Deploy\\(Render/Fly)};
