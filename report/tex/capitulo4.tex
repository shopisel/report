...
\begin{tikzpicture}[
    ...
    service/.style={rectangle, draw=black, rounded corners=4pt, fill=white,
                    minimum width=2.6cm, minimum height=0.85cm, font=\small, align=center},
    ...
    gw/.style={rectangle, draw=black, rounded corners=4pt, fill=yellow!20,
               minimum width=3cm, minimum height=0.85cm, font=\small\bfseries, align=center},
...
\node[client, right=2.2cm of web] (mobile) {Mobile App\\(React Native)};

%--- Gateway ---
\node[gw, below=1.0cm of web, xshift=1.4cm] (nginx) {Nginx Gateway};

%--- Serviços ---
\node[service, below left=1.4cm and 2.0cm of nginx] (list)    {List\\Service};
\node[service, left=0.5cm of list]                  (account) {Account\\Service};
\node[service, right=0.5cm of list]                 (products){Products\\Service};
\node[service, right=0.5cm of products]             (prices)  {Prices\\Service};
\node[service, right=0.5cm of prices]               (stores)  {Stores\\Service};

%--- Bases de dados ---
\node[db, below=0.7cm of account]  (db-acc)  {PostgreSQL};
\node[db, below=0.7cm of list]     (db-list) {PostgreSQL};
\node[db, below=0.7cm of products] (db-prod) {PostgreSQL};
\node[db, below=0.7cm of prices]   (db-pr)   {PostgreSQL};
\node[db, below=0.7cm of stores]   (db-st)   {PostgreSQL};

%--- Serviços externos ---
\node[ext, right=2.8cm of nginx]  (keycloak) {Keycloak\\(Auth)};
\node[ext, below=0.5cm of keycloak] (fcm)    {Firebase\\FCM};

%--- MatchQueue ---
\node[service, below=2.8cm of prices] (mq) {MatchQueue\\Service};
\node[db, left=0.6cm of mq]  (mongo)   {MongoDB};
\node[db, right=0.6cm of mq] (db-mq)   {PostgreSQL};

%--- Scrapers ---
\node[service, below=1.3cm of mq, xshift=-2cm] (sc-cont) {Scraper\\Continente};
\node[service, right=0.8cm of sc-cont]          (sc-pd)   {Scraper\\Pingo Doce};

%--- FCM Worker ---
\node[service, below=1.3cm of db-mq] (fcmw) {FCM\\Worker};

%--- Ligações cliente → gateway ---
\draw[arr] (web.south) -- (nginx.north west);
\draw[arr] (mobile.south) -- (nginx.north east);

%--- Gateway → serviços ---
\draw[arr] (nginx.south) -- (list.north);
\draw[arr] (nginx.south) -- (products.north);
\draw[arr] (nginx.south) -- (prices.north);
\draw[arr] (nginx.south) -- (stores.north);
\draw[arr] (nginx.south) -- (account.north);
...
%--- Auth ---
\draw[darr] (nginx.east)    -- (keycloak.west);
\draw[darr] (list.east)     -| (keycloak.south);
