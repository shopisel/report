...
\begin{itemize}
    \item \textbf{ShoppingListEntity}: representa uma lista de compras, com um identificador único, o identificador do proprietário (\texttt{OwnerId}, derivado do claim \texttt{sub} do token \ac{JWT}), o nome da lista, a data de criação e um token de versão (\texttt{Version}) utilizado para controlo de concorrência optimista.
...
\subsubsection{Tratamento de erros e concorrência optimista}
\label{sec:4.2.1.1}
...
Para resolver este problema, foi adoptada uma estratégia de concorrência optimista. Cada lista possui um identificador de versão do tipo \texttt{Guid}, gerido pela aplicação e persistido na base de dados como token de concorrência do Entity Framework Core. Sempre que uma lista é criada ou actualizada com sucesso, recebe uma nova versão; nas operações de actualização e eliminação, o serviço compara a versão recebida no header \texttt{If-Match} com a versão actualmente armazenada. Se coincidirem, a operação prossegue; se outro cliente tiver alterado a lista entretanto, o Entity Framework Core detecta o conflito e a API responde com o código HTTP 409 (\textit{Conflict}).

Esta abordagem foi preferida a alternativas como \textit{rowversion} específico de SQL Server, o token nativo \texttt{xmin} do PostgreSQL ou locking pessimista com transacções longas. A opção escolhida mantém o comportamento portável entre PostgreSQL, utilizado em produção, e SQLite, utilizado nos testes de integração, sem bloquear recursos de base de dados enquanto o utilizador interage com a interface.
...
Para permitir a publicação da aplicação web sob diferentes identidades visuais sem manter ramificações de código separadas, foi implementado um sistema de personalização em tempo de \textit{build}. O objectivo é alterar nome, slogan, logótipo e esquema de cores da marca apenas através de parâmetros injectados no processo de construção e \textit{deployment}, mantendo o código-fonte partilhado focado na lógica funcional da aplicação.
...
No arranque da aplicação, as cores resolvidas são injectadas como propriedades CSS personalizadas no documento, o que permite que as classes utilitárias do Tailwind e os componentes existentes adoptem imediatamente a nova paleta. Elementos como a barra lateral, o ecrã de \textit{splash}, o \textit{onboarding} e os controlos de navegação passam a reflectir a marca configurada sem alterações estruturais ao código.

O fluxo completo de personalização percorre o pipeline de \ac{CI/CD}: o workflow de publicação Docker suporta execução manual com inputs opcionais para os parâmetros de marca; estes valores são passados como argumentos de construção da imagem, convertidos em variáveis de ambiente durante o \textit{build} do Vite e incorporados de forma estática no \textit{bundle} final. Quando o workflow corre automaticamente após integração no branch principal, os inputs assumem valores vazios e a aplicação mantém a identidade por defeito. Desta forma, a activação de uma nova variante white label reduz-se a executar o workflow de \textit{deploy} com os parâmetros desejados, sem necessidade de editar o repositório.
