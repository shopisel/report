...
\begin{itemize}
    ...
    \item \texttt{POST /prices} — regista ou atualiza o preço de um produto numa loja (utilizado internamente pelo MatchQueue);
...
Este serviço é implementado como uma única aplicação ASP.NET Core, mas suporta dois modos de execução distintos, selecionados através de um argumento de linha de comandos (\texttt{-{}-mode worker}):

\begin{itemize}
    \item \textbf{Modo serviço} (por omissão): a aplicação é publicada como imagem Docker e mantém-se sempre ativa, expondo o endpoint \texttt{POST /matchqueue/trigger}, que um scraper pode invocar via \ac{HTTP} para desencadear o processamento (opção configurada por variável de ambiente);
    \item \textbf{Modo worker} (\texttt{-{}-mode worker}): a mesma aplicação é compilada e executada diretamente num \textit{runner} efémero do GitHub Actions, sem publicação de imagem Docker nem endpoint \ac{HTTP} exposto. Neste modo, o processo lê o \textit{payload} do evento \texttt{repository\_dispatch} recebido do Scraper Orchestrator e invoca internamente a mesma lógica de negócio do endpoint de \textit{trigger}, terminando após a conclusão.
\end{itemize}

Os dois modos partilham a mesma lógica de negócio, variando apenas a forma como são invocados e o ciclo de vida do processo: o modo serviço mantém-se permanentemente disponível, enquanto o modo worker segue um modelo \textit{run-to-completion}, orquestrado inteiramente pelo GitHub Actions. É este segundo modo, e não o endpoint \ac{HTTP}, que é utilizado no fluxo principal da Figura~\ref{fig:pipeline}.
...
Após a conclusão do processamento em modo worker, o próprio workflow do MatchQueue disponibiliza um evento \texttt{repository\_dispatch} (\texttt{matchqueue\_finished}) para o repositório \texttt{fcm-notification-worker}, encadeando a normalização e o envio de notificações sem necessidade de um agendamento independente.
...
Após a conclusão da sincronização com o MongoDB, o scraper pode opcionalmente disparar o processamento no MatchQueue através de um pedido \texttt{POST /matchqueue/trigger}, configurado por variável de ambiente.
