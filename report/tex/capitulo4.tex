...
\begin{tikzpicture}[
    ...
    db/.style={cylinder, draw=black!70, fill=gray!15, shape border rotate=90,
               minimum width=2.0cm, minimum height=0.7cm, font=\scriptsize, align=center},
    ext/.style={rectangle, draw=black!70, rounded corners=4pt, fill=orange!15,
                minimum width=2.6cm, minimum height=0.9cm, font=\small, align=center},
    gw/.style={rectangle, draw=black, rounded corners=4pt, fill=yellow!20,
               minimum width=3.4cm, minimum height=0.9cm, font=\small\bfseries, align=center},
    arr/.style={-{Stealth[length=6pt]}, thick},
    darr/.style={<->, thick, dashed}
]

%--- Linha 1: Clientes ---
\node[client] (web)    at (1.0, 0)    {Web App\\(React + Vite)};
\node[client] (mobile) at (5.0, 0)    {Mobile App\\(React Native)};

%--- Linha 2: Gateway + externos ---
\node[gw]  (nginx) at (3.0, -2.0)   {Nginx Gateway};
\node[ext] (kc)    at (11.5, -2.0)  {Keycloak\\(Auth)};
\node[ext] (fcm)   at (11.5, -3.8)  {Firebase\\FCM};

%--- Linha 3: Serviços (espaçamento de 3 cm entre centros) ---
\node[service] (account)  at (-3.0, -4.2) {Account\\Service};
\node[service] (list)     at ( 0.0, -4.2) {List\\Service};
\node[service] (products) at ( 3.0, -4.2) {Products\\Service};
\node[service] (prices)   at ( 6.0, -4.2) {Prices\\Service};
\node[service] (stores)   at ( 9.0, -4.2) {Stores\\Service};

%--- Linha 4: Bases de dados ---
\node[db] (db-acc)  at (-3.0, -6.0) {PostgreSQL};
\node[db] (db-list) at ( 0.0, -6.0) {PostgreSQL};
\node[db] (db-prod) at ( 3.0, -6.0) {PostgreSQL};
\node[db] (db-pr)   at ( 6.0, -6.0) {PostgreSQL};
\node[db] (db-st)   at ( 9.0, -6.0) {PostgreSQL};

%--- Linha 5: MatchQueue ---
\node[db]      (mongo)  at ( 0.5, -8.0) {MongoDB};
\node[service] (mq)     at ( 3.5, -8.0) {MatchQueue\\Service};
\node[db]      (db-mq)  at ( 7.0, -8.0) {PostgreSQL};

%--- Linha 6: Scrapers + FCM Worker ---
\node[service] (sc-cont) at (-0.5, -10.0) {Scraper\\Continente};
\node[service] (sc-pd)   at ( 2.5, -10.0) {Scraper\\Pingo Doce};
\node[service] (fcmw)    at ( 7.0, -10.0) {FCM\\Worker};

%--- Clientes → Gateway ---
\draw[arr] (web.south)    -- (nginx.north west);
\draw[arr] (mobile.south) -- (nginx.north east);

%--- Gateway → Serviços (barra de distribuição horizontal) ---
\coordinate (bus) at (3.0, -3.0);
